async function uploadFile(id) {
    ...
    formData.append("projectAidId", id); // Add the string ID

    uploadStatus.innerHTML = "جاري التحميل...";

    try {
        $.ajax({
            url: "/ProjectAid/Import",
            type: "POST",
            data: formData,
            contentType: false,
            processData: false,
            xhr: function () {
                ...
                myXhr.responseType = "blob"; // Ensure response is treated as a Blob
                return myXhr;
            },
                ...
                if (xhr.status === 200) {
                    if (contentType.includes("application/json") || contentType.includes("text/plain")) {
                        ...
                        reader.onload = function () {
                            $("#uploadStatus").html(`<span class="text-success">${reader.result}</span>`);
                        };
                        ...
                        $("#uploadStatus").html('<span class="text-success">تم التحميل، تم تنزيل ملف الأخطاء</span>');
                    }
                    else {
                        $("#uploadStatus").html('<span class="text-danger">استجابة غير معروفة من الخادم</span>');
                    }
                }
            },
            error: function () {
                $("#uploadStatus").html('<span class="text-danger">فشل التحميل، خطأ في الخادم</span>');
            }        });
    } catch (error) {
...
var headers = ["الاسم", "رقم هوية الزوج", "رقم الهاتف", "عدد الأفراد", "الحالة الإجتماعية","حالة طلب الإغاثة"];
